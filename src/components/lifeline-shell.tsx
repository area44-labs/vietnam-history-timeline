  ...
  return (
    ...
    >
      ...
      >
        ...
        {children ? <div className="flex items-center gap-8">{children}</div> : null}
      </div>
    </nav>
  );
}
...
export function LifelineStage({
  ...
  return (
    <main className={cn("min-h-0 flex-1 overflow-y-auto pt-16 md:overflow-hidden", className)}>
      {children}
    </main>
  );
}
...
export function LifelineFooter({
  ...
  return (
    ...
    >
      <div className={cn(CONTAINER, "h-16 justify-between gap-6", containerClassName)}>
        {children}
      </div>
    </footer>
  );
